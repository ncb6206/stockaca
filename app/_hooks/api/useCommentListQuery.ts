...
import { getCommentListNext, getCommentListFirst } from '@/app/_api/post/getCommentList';
import { IPostData, IPostId } from '@/app/_types/post';

const useGetCommentListQuery = ({ postId }: IPostId) => {
  ...
  >({
    ...
    queryFn: ({ pageParam }) => {
      return pageParam ? getCommentListNext(postId, pageParam) : getCommentListFirst(postId);
    },
...
export default useGetCommentListQuery;
