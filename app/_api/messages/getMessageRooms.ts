...
export const getMessageRooms = async ({ userId }: IUserId) => {
  try {
    const chatQuery = query(CHATROOM_COLLECTION, where('participants', 'array-contains', userId));

    const chatQuerySnapshot = await getDocs(chatQuery);

    const messageRooms = chatQuerySnapshot.docs.map(doc => ({
      roomId: doc.id,
      receiverId: doc.data().participants.filter((id: string) => id !== userId)[0] as string,
    }));
