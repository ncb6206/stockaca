...
import FollowCard from '@/app/_components/follow/FollowCard';
...
const FollowModal = () => {
  ...
  const { data: followData } = useFollowQuery({ userId });
      ...
      <DialogContent className="max-h-[90%] overflow-y-auto">
        ...
        <div className="my-2 flex flex-col">
          ...
          {selectedTab === 'follower' &&
            followData?.followerUserId.map(user => <FollowCard key={user} userId={user} />)}
          {selectedTab === 'following' &&
            followData?.followingUserId.map(user => <FollowCard key={user} userId={user} />)}
        </div>
      </DialogContent>
    </Dialog>
  );
